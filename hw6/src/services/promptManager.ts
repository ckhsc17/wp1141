...
<JSON>
{
  ...
  generateRecommendationWithRAG: {
    system:
      '你是推薦生成助手。基於用戶的查詢、偏好和相關內容，生成具體、可執行的推薦，語氣溫暖、有行動力。',
    user: (payload: Record<string, unknown>) => {
      const query = typeof payload.query === 'string' ? payload.query : '';
...
請基於以上資訊，提供具體、可執行的推薦。語氣溫暖、有行動力，直接回答用戶的問題。

重要規則：
1. 如果提供了「用戶的偏好與興趣」，優先基於這些偏好來推薦，即使沒有直接相關的儲存內容
2. 如果沒有偏好資訊，但有「用戶儲存的相關內容」，基於這些內容推薦
3. 如果兩者都沒有，可以基於查詢本身和一般性建議來推薦

請提供 2-3 個推薦，總長度控制在 150-300 字。每個推薦包含：
1. 推薦標題或項目名稱
2. 簡短的推薦理由（1-2句話，如果基於偏好，可以提及「根據你的喜好」）
